...
//        self.color = [[UIColor blackColor] colorWithAlphaComponent:0.8]; // sponzy
//        self.highlightedColor = [[UIColor darkGrayColor] colorWithAlphaComponent:0.8]; // sponzy
        
        self.color = [[UIColor blackColor] colorWithAlphaComponent:1.0];
        self.highlightedColor = [[UIColor darkGrayColor] colorWithAlphaComponent:1.0];
...
//    [targetV setBackgroundColor:[UIColor greenColor]];
//    [self.view.superview addSubview:targetV];
//    NSLog(@"frame %f %f %f %f", targetRect.origin.x, targetRect.origin.y, targetRect.size.width, targetRect.size.height);
//    // test end
    UIView *superView = self.view.superview;
    
    float visible_origin_under_nav_bar = targetRect.origin.y - self.navigationBarHeight - self.statusBarHeight;
    if (arrowDirection == QBPopupMenuArrowDirectionDefault) {
//        NSLog(@"targetRect.origin.y %f", targetRect.origin.y);
//        NSLog(@"menu.height %f", self.height);
//        NSLog(@"arrow.size %f", self.arrowSize);
//        NSLog(@"popupMenuInsets.top %f", self.popupMenuInsets.top);
        
        if ((visible_origin_under_nav_bar - (self.height + self.arrowSize)) >= self.popupMenuInsets.top) {
            arrowDirection = QBPopupMenuArrowDirectionDown;
        }
        else if ((visible_origin_under_nav_bar + targetRect.size.height + (self.height + self.arrowSize)) < (superView.bounds.size.height - self.popupMenuInsets.bottom)) {
            arrowDirection = QBPopupMenuArrowDirectionUp;
        }
        else {
            CGFloat left = targetRect.origin.x - self.popupMenuInsets.left;
            CGFloat right = superView.bounds.size.width - (targetRect.origin.x + targetRect.size.width + self.popupMenuInsets.right);
    ...
    // Create overlay view
//    NSLog(@"self.overlayView %@", self.overlayView);
    self.overlayView = ({
